...
import { ArrowLeft, Save, Trash2, MapPin, Plus } from 'lucide-react';
...
interface Service {
    ...
    delivery_tiers?: { from_km: string; to_km: string; fee: string }[];
...
export default function ServicesEdit({ service, professionals }: { service: Service, professionals: Professional[] }) {
    const { data, setData, put, delete: destroy, processing, errors } = useForm({
        ...
        professional_ids: service.professionals ? service.professionals.map(p => p.id.toString()) : [] as string[],
        available_online: service.available_online,
        available_home_service: service.available_home_service,
        home_service_surcharge: service.home_service_surcharge ? service.home_service_surcharge.toString() : '',
        home_service_radius_km: service.home_service_radius_km ? service.home_service_radius_km.toString() : '',
        home_service_latitude: service.home_service_latitude,
        home_service_longitude: service.home_service_longitude,
        delivery_tiers: (service.delivery_tiers || []) as DeliveryTier[],
    ...
    const handleDelete = () => {
        destroy(`/business/services/${service.id}`);
    };
            ...
            <div className="max-w-2xl mx-auto space-y-6 p-4">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <h1 className="text-3xl font-bold bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text text-transparent">Editar Servicio</h1>
                    </div>
                            ...
                            <div className="flex items-center justify-between p-4 border rounded-lg bg-muted/20">
                                <div className="space-y-0.5">
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        {data.is_active ? 'El servicio está visible y se puede reservar.' : 'El servicio está oculto.'}
                                    </p>
                                </div>
                                <Switch
                                            ...
                                            {data.delivery_tiers.length > 0 ? (
                                                <div className="space-y-4">
                                                    {data.delivery_tiers.map((tier, index) => (
                                                        <div key={index} className="flex flex-col sm:flex-row items-end gap-2 p-3 border rounded-lg bg-muted/10">
